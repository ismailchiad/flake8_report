...
class TournamentView(View):
    ...
    def get_tournament_info(self):
        ...
        self.rounds = self.get_valid_int_input(
            "Nombre de tours (optionnel, défaut à 4): ",
            4)  # IF EMPTY, SET TO 4
    ...
    def select_tournament(self, tournaments):
        ...
        for i, tournament in enumerate(tournaments_reversed):
            ...
            print(f"{i + 1} - {tournament['name']} à {tournament['location']} du {tournament['start_date']} au {
                tournament['end_date']} - Rounds: {tournament['rounds']} - Inscrits: {registered_players}")
        while True:
            try:
                ...
                if tournament_index < 0 or tournament_index >= len(tournaments):
                    ...
                    # On retourne l'ID à ne pas confondre avec les options en affichage (pas
                    # les mêmes valeurs)
